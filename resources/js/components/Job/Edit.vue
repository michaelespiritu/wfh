<template>
    <div>
        ...
        <div class="form-group row my-4">
            ...
            <div class="col-sm-12">
            <button type="button" class="btn btn-outline-primary" @click="addSkill(skills)" v-if="skills.length === 0">Add Skill</button>
            <div class="input-group mb-2" v-for="(skill, index) in skills" :key="index">
                <div class="input-group-prepend">
                <div class="input-group-text" @click="addSkill(skills)"><i class="fa fa-plus"></i></div>
                </div>
                <input v-model="skills[index]" type="text" v-validate="'required'" class="form-control" id="skills" name="skills" placeholder="Skills">
                <div class="input-group-prepend">
                <div class="input-group-text" @click="deleteSkill(skills, index)"><i class="fa fa-minus"></i></div>
                </div>
            </div>
        ...
        <div class="form-group row my-4">
            ...
            <div class="col-sm-12">
            <button type="button" class="btn btn-outline-primary" @click="addSkill(emails)" v-if="emails.length === 0">Add Email</button>
            <div class="input-group mb-2" v-for="(email, index) in emails" :key="index">
                <div class="input-group-prepend">
                <div class="input-group-text" @click="addSkill(emails)"><i class="fa fa-plus"></i></div>
                </div>
                <input v-model="emails[index]" type="text" v-validate="'required'" class="form-control" id="emails" name="emails" placeholder="emails">
                <div class="input-group-prepend">
                <div class="input-group-text" @click="deleteSkill(emails, index)"><i class="fa fa-minus"></i></div>
                </div>
            </div>
...
export default {
    ...
    created() {
        ...
        this.category = this.job.category.id
        this.skills = this.job.skills
        this.regionRestriction = this.job.region_restriction
        this.emails = this.job.notify_email
        this.employmentType = this.job.type
        this.budgetType = this.job.budget_raw.type
        this.budgetAmount = this.job.budget_raw.amount
        ...
        submitDetails () {
            axios.patch(`${this.job.path}`, {
                ...
                this.$swal({
                    type: 'error',
                    title: 'Ooppss!',
                    html: `Something went wrong. Please Try again. <br> ${ err.response.data.hasOwnProperty('error') ? '<span class="text-danger">Tip</span>: ' + err.response.data.error : ''}`
                })
            })
        }
    }
