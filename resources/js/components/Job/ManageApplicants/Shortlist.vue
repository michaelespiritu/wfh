<template>
    <div class="col-4">
        <p class="h3 text-center">Shortlisted</p>
        <hr>
        <draggable 
                v-model="shortlist" 
                class="dragArea"
            ...
            <div 
                v-for="(applicant, index) in shortlist"
                :key="index"
                class="col-sm-12" 
                >
                <div 
...
export default {
    ...
    methods: {
        applicantUpdate (evt) {
            if (evt.hasOwnProperty('added')) {
                this.updateOrder(
                    ...
                    `${evt.added.element.user.name} has been moved to Shortlist`
                )
            }
        },
        updateOrder (target, message) { 
            ...
            .then(response => {
                this.$store.commit('SET_APPLCIANT_SHORTLIST', response.data.applicants)
                this.successAlert(
                    message, 
                    null,
                    {
